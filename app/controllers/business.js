...
exports.getFlowList = async(ctx) => {
  ...
  let data;
  if(key_words) {
    data = await Business.searchFlowByName(manager_id, key_words, page, limit);
  }else {
    data = await Business.getFlowList(manager_id, page, limit);
  }
  if(data) await Business.updateFlowStatus(data.list);
